...
def find_entity(
...
) -> Optional[WebSocketClientSwitchEntity]:
    ...
    for entity in device_proxy.platform_entities.values():
        if platform == entity.PLATFORM:
            return cast(WebSocketClientSwitchEntity, entity)
    return None
...
def get_group_entity(
    ...
    return cast(WebSocketClientSwitchEntity, group_proxy.group_entities.get(entity_id))
...
async def test_switch(
    ...
    entity: WebSocketClientSwitchEntity = find_entity(client_device, Platform.SWITCH)
    ...
    with patch(
        ...
        await controller.switches.turn_on(entity.info_object)
    ...
    with (
        ...
        await controller.switches.turn_off(entity.info_object)
        await server.async_block_till_done()
        assert entity.state["state"] is True
        assert len(cluster.request.mock_calls) == 1
        assert cluster.request.call_args == call(
